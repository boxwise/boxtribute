...
const DistributionList = ({
  ...
  const pastDistroEventsInReturnState = pastNonCompletedDistroEvents.filter(
    (el) => el.state === DistributionEventState.Returned
  );

  const pastNonCompletedDistroEventsInNonReturnState = pastNonCompletedDistroEvents.filter(
    el => el.state !== DistributionEventState.Returned
  )
  ...
  const hasDistroEventsInReturnState =
    pastDistroEventsInReturnState.length > 0;

  const hasPastNonCompletedDistroEventsInNonReturnState = pastNonCompletedDistroEventsInNonReturnState.length > 0;
    ...
    <VStack>
      ...
      {hasPastDistroEvents && (
        <>
          ...
          {hasPastNonCompletedDistroEventsInNonReturnState && (
            <>
