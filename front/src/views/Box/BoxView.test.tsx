  ...
  result: {
    data: {
      box: generateMockBox({ 
        labelIdentifier: "deletedprod123",
        qrCode: null,
        product: { ...productBasic1, deletedOn: "2023-01-01T00:00:00Z" },
      }),
      shipments: null,
    },
  },
...
const createQrCodeMutationError = {
  request: {
    query: CREATE_QR_CODE_MUTATION,
    variables: {
      boxLabelIdentifier: "deletedprod123",
    },
  },
...
it("QR.1 - Display warning alert for box without QR code and create button", async () => {
  ...
  expect(screen.getByText(/missing qr code/i)).toBeInTheDocument();
  expect(screen.getByText(/this box has no qr code associated with it/i)).toBeInTheDocument();
