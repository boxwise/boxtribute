        ...
        if (boxFormValues.numberOfItems > boxData?.numberOfItems) {
          ...
          createBoxFromBoxMutation({
            ...
            .then((mutationResult) => {
              if (mutationResult?.errors) {
                triggerError({
                  message: "Error: Could not create box from box",
                });
              } else {
                ...
                if (result?.__typename === "Box") {
                  createToast({
                    ...
                    type: "success",
                    message: `Successfully created in ${result.location?.name}`,
                  });
                  onMinusClose();
                } else {
                  triggerError({
                    message: `Error: ${result?.name || "Could not create box from box"}`,
                  });
                }
              }
            })
