...
;""",
    ...
    single_base_user_group_ids = [row["cms_usergroups_id"] for row in result]
...
;""",
    ...
    result = execute_sql(
        int(base_id),
        query="""SELECT * FROM cms_usergroups_camps cuc WHERE cuc.camp_id = %s;""",
    )
    ...
    result = execute_sql(
        int(base_id),
        query="""SELECT * FROM cms_functions_camps cfc WHERE cfc.camps_id = %s;""",
    )
...
;""",
        ...
        LOGGER.info(", ".join([row["naam"] for row in result]))
...
def _update_user_data_in_database(
    *, base_id, single_base_users, single_base_user_role_ids, single_base_user_group_ids
    ...
    execute_sql(
        int(base_id),
        query="""UPDATE camps SET deleted = UTC_TIMESTAMP() WHERE id = %s;""",
    )
...
;""",
    ...
    execute_sql(
        int(base_id),
        query="""DELETE cuc FROM cms_usergroups_camps cuc WHERE cuc.camp_id = %s;""",
    )
    # Remove rows with base ID from cms_functions_camps table
    execute_sql(
        int(base_id),
        query="""DELETE cfc FROM cms_functions_camps cfc WHERE cfc.camps_id = %s;""",
    )
    ...
    if single_base_user_role_ids:
        ...
        execute_sql(
            single_base_user_role_ids,
            query="""DELETE FROM cms_usergroups_roles WHERE auth0_role_id IN %s;""",
        )
