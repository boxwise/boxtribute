...
def test_remove_base_access(usergroup_data):
    ...
    data = execute_sql(
        query="SELECT camp_id,cms_usergroups_id from cms_usergroups_camps;"
    )
    ...
    data = execute_sql(
        query="SELECT camps_id,cms_functions_id from cms_functions_camps;"
    )
    assert [(r["camps_id"], r["cms_functions_id"]) for r in data] == [(2, 1)]

    # Verify that all entries related to non-admin usergroups are removed from
    # cms_usergroups_roles
    data = execute_sql(
        query="SELECT cms_usergroups_id,auth0_role_name from cms_usergroups_roles;"
    )
    assert [(r["cms_usergroups_id"], r["auth0_role_name"]) for r in data] == [
        (1, "administrator"),
    ...
    # cms_usergroups_functions
    data = execute_sql(
        query="SELECT cms_usergroups_id,cms_functions_id from cms_usergroups_functions;"
    )
    assert [(r["cms_usergroups_id"], r["cms_functions_id"]) for r in data] == [
        (1, 1),
        (6, 1),
        (7, 1),
    ]
